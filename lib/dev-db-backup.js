...
function mirrorBackupCopy(root, srcPath, filename) {
  const mirrorDir = getBackupMirrorDir(root);
  if (!mirrorDir) {
    return { ok: true, skipped: true, reason: "BACKUP_MIRROR_DIR not configured" };
  }
  try {
    const dest = path.join(mirrorDir, filename);
    copyFileSafe(srcPath, dest);
    const meta = statBackupFile(dest, filename);
    return {
      ok: true,
      ...meta,
      relPath: path.relative(root, dest).replace(/\\/g, "/"),
      backupDirectory: mirrorDir
    };
  } catch (e) {
    return {
      ok: false,
      error: e.message || "Mirror backup failed",
      backupDirectory: mirrorDir
    };
  }
}
...
function createLocalBackup(root) {
  ...
  const mirror = mirrorBackupCopy(root, result.localPath, result.filename);
  let output = `Created local backup: ${result.localPath} (${result.sizeHuman})`;
  if (mirror.skipped) {
    /* primary only */
  } else if (mirror.ok) {
    output += `. Mirror copy: ${mirror.localPath} (${mirror.sizeHuman})`;
  } else {
    output += `. Mirror copy failed: ${mirror.error}`;
  }

  return {
    ok: true,
    backup: result,
    mirrorBackup: mirror.skipped ? null : mirror,
    mirrorWarning: mirror.ok || mirror.skipped ? undefined : mirror.error,
    backupDirectory: result.backupDirectory,
    mirrorDirectory: mirror.skipped ? null : mirror.backupDirectory,
    livePath,
    output
  };
}
