...
/** Optional second copy (e.g. D:\\paint-market\\backups) — BACKUP_MIRROR_DIR in .env */
function getBackupMirrorDir(root) {
  const envDir = process.env.BACKUP_MIRROR_DIR;
  if (!envDir || !String(envDir).trim()) return null;
  const trimmed = String(envDir).trim();
